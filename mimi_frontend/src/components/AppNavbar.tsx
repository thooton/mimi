...
function guestNext(path: string): string {
  const here = typeof window === 'undefined' ? '/learn' : window.location.pathname;
    ...
    const load = () => {
      ...
      fetchProfile(username)
        ...
        .catch(() => {
          ...
          if (!cancelled && first) setState({ me: null, settled: true });
        });
    };
    load();
      ...
      <div className="shell topbar-inner">
        ...
        <nav className="topnav" aria-label="Main">
          {NAV.map((item) => {
            ...
            const cls = isActive ? 'topnav-link is-active' : 'topnav-link';
            ...
            if (item.id === 'practice') {
              ...
              return (
                <Popover
                  ...
                  modifiers={{
                    offset: { enabled: true, options: { offset: [0, 7] } },
                  }}
                  ...
                  renderTarget={({ isOpen, ref, ...targetProps }) => (
                    <button
                      ...
                      type="button"
                      className={isOpen ? `${cls} is-open` : cls}
                  ...
                  content={
                    <Menu role="menu" aria-label={item.label}>
                      {menu.map((sub) => (
                        <MenuItem
                          key={sub.label}
                          href={sub.href}
        ...
        <div className="topbar-right">
          ...
          {guest && settled && (
            <div className="topbar-auth">
              <a className="bp6-button" href={guestNext('/login')}>Sign in</a>
              <a className="bp6-button bp6-intent-primary" href={guestNext('/signup')}>
                Create account
              </a>
            </div>
          )}
          ...
          {ready && user && me && !guest && (
            <Popover
              ...
              content={
                <Menu>
                  <MenuItem icon="person" text="Profile" href="/profile" />
                  ...
                  <MenuItem icon="envelope" text="Inbox" href="/inbox" />
                  ...
                  <MenuItem
                    ...
                    text="Sign out"
                    onClick={() => {
                      signOut().finally(() => window.location.assign('/login'));
                    }}
                  />
                </Menu>
              }
            >
              <button className="account-btn" type="button">
                ...
                <span className="account-name">{me.display}</span>
          ...
          {ready && !user && (
            <div className="topbar-auth">
              <a className="bp6-button" href="/login">Sign in</a>
              <a className="bp6-button bp6-intent-primary" href="/signup">Create account</a>
            </div>
          )}
          ...
          >
            <Icon icon={menuOpen ? 'cross' : 'menu'} size={18} aria-hidden />
          </button>
        </div>
      </div>
        ...
        <nav className="mobilenav" id="mobilenav" aria-label="Main">
          {NAV.map((item) => {
            ...
            if (item.id === 'practice') {
              ...
              return (
                <div className="mobilenav-group" key={item.id}>
                  <span className="mobilenav-heading">{item.label}</span>
            ...
            return (
              <a
                key={item.id}
                className={isActive ? 'mobilenav-link is-active' : 'mobilenav-link'}
                href={item.href}
                ...
                onClick={external ? () => setMenuOpen(false) : undefined}
              >
                {item.label}
                {external && (
                  <Icon className="topnav-external" icon="share" size={11} aria-hidden />
